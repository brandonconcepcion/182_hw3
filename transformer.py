...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        position_shape = (1, sequence_length, self.hidden_size) # fill in the other two dimensions (None's)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        attn = self.self_attention((norm_inputs, norm_inputs), self_attention_mask  # Pass the mask as a keyword argument
        )
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):
        ...
        norm_decoder_inputs = self.self_norm(decoder_inputs)  # Normalize decoder inputs
        target_selfattn = self.self_attention((norm_decoder_inputs, norm_decoder_inputs),
                                              mask=self_attention_mask)  # Self-attention
        res_target_self_attn = decoder_inputs + target_selfattn  # Residual connection for self-attention
        ...
        norm_target_selfattn = self.cross_norm_target(res_target_self_attn)  # Normalize target self-attention outputs
        norm_encoder_outputs = self.cross_norm_source(encoder_outputs)  # Normalize encoder outputs
        encdec_attention = self.cross_attention((norm_target_selfattn, norm_encoder_outputs),
                                                mask=cross_attention_mask)  # Cross-attention
        res_encdec_attention = res_target_self_attn + encdec_attention  # Residual connection for cross-attention
...
class Transformer(nn.Module):
    ...
    def forward(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True, shift_target_sequence_right=True):
        ...
        decoder_output = self.decoder(
            target_sequence,  # Pass target_sequence directly if this matches the expected input name
            encoder_output,
