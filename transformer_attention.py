...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        similarity = th.matmul(queries, keys.transpose(-2, -1)) / th.sqrt(th.tensor(key_dim, dtype=th.float32))  # Compute the similarity according to the QKV formula
...
class MultiHeadProjection(nn.Module):
    ...
    def _combine_heads(self, tensor):
        ...
        new_feature_size = tensor.shape[2] * feature_size # What is the new feature size, if we combine all the heads
